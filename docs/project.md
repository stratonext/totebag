...
summary, instructions, tools to restore, notes, links, docs, assets, the open task list, and the
recent journal. It restores any missing tools, works the task list, and writes knowledge back as
it learns. The next session (or a different LLM, or a different vendor) inherits all of it.
...
- **The task list stays honest.** Add work before doing it; remove each task when done.
  Completing a task records what was accomplished in the journal, then drops the task.
...
(`project | note | link | document | list | asset | tool | task | journal`).
...
$TOTEBAG_ROOT/
  ...
  workspaces/wsp_abc/
    ...
    projects/prj_abc/
      ...
      log.md                        # OKF reserved work log (§9), appended
```
...
`tool`, `journal`, plus top-level `init`, `config` (shows the active sink and path), and `search`
(substring).
