...
**docs** (markdown), **notes**, **links**, **assets** (files), **tools/skills** it needs, a
**task list** (work to do), and a day-by-day **journal** of work.
...
- **Write back as you go.** The moment you learn a durable fact, make a decision, hit a gotcha, or
  ...
  for anything worth keeping, the **journal** for progress, a **task** for work still to do.
...
- **`totebag project context`** - the **restore blob**: description, instructions, **tools/skills to
  restore**, notes, links, every doc, the asset inventory, the **open task list**, and the
  **recent journal**, assembled into one markdown document. This is what you read to rehydrate a
  project. Start here.
...
## Track work on the task list

Put work you intend to do on the task list, and **remove each task the moment it's done** so
the list always shows only what's still outstanding. Completing a task records what you
accomplished in the journal, then drops it:
...
totebag task done   tsk_yyy --description "webhooks handled with idempotency keys"
# `task done` journals the work and removes the task. `task rm tsk_yyy --confirm` drops one un-done.
```

Each task keeps its own attachments, so a task carries its own working files with it.

## Journal what you did, day by day

The journal is a running, day-by-day log so a future session sees progress at a glance. It is
**written automatically** - every `task done` appends an entry - so there is no manual add and no
log to keep up by hand; it only ever holds reliably-recorded completed work. Read it back with:

```bash
totebag journal list                              # newest day first
```
...
- Write knowledge that outlives the session (decisions, gotchas, where things live), not
  transient chatter.
- Keep the task list honest: add work before you do it, remove it (`task done`) when finished.
