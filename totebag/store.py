...
class Store:
    ...
    def complete_task(self, pid: str, tid: str, description: str) -> None:
        """Mark work done: record it in the journal, then drop the task from the list."""
        _require_description(description, "completed task")
        task = self.sink.load_task(pid, tid)
        self.add_journal(pid, f"Completed '{task.title}': {description}")
        self.remove_task(pid, tid)

    # --- journal -----------------------------------------------------------
    def add_journal(self, pid: str, text: str, date: str | None = None) -> None:
        self.sink.load_project(pid)  # ensure exists
        self.sink.append_journal(pid, text, date)

    def list_journal(self, pid: str) -> list[JournalEntry]:
        return self.sink.load_journal(pid)
