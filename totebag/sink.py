...
class Sink(ABC):
    ...
    # --- journal -----------------------------------------------------------
    @abstractmethod
    def append_journal(self, pid: str, text: str, date: str | None = None) -> None: ...
    @abstractmethod
    def load_journal(self, pid: str) -> list[JournalEntry]: ...
...
class OKFSink(Sink):
        ...
        $ROOT/
          ...
          workspaces/<wsp_id>/
            ...
            projects/prj_x/
              ...
              log.md                        # OKF reserved log (§9); one per project
    """
    ...
    # --- journal = the OKF reserved log.md (§9) ----------------------------
    def _log_path(self, pid: str) -> str:
        return self._p("projects", pid, "log.md")

    @staticmethod
    def _parse_log(text: str) -> dict[str, list[str]]:
        """`## YYYY-MM-DD` sections -> {date: [entry, ...]} (leading `- ` stripped)."""
        ...
        for line in text.splitlines():
            if line.startswith("## "):
                ...
                days[cur].append(line[2:])
        return days

    @staticmethod
    def _dump_log(days: dict[str, list[str]]) -> str:
        out = ["# Log", ""]
        for date in sorted(days, reverse=True):  # §9: newest date first
            out.append(f"## {date}")
            out.extend(f"- {item}" for item in days[date])
            out.append("")
        return "\n".join(out).rstrip() + "\n"
    ...
    def append_journal(self, pid: str, text: str, date: str | None = None) -> None:
        ...
        days = self._parse_log(self._read_text(path)) if self.fs.exists(path) else {}
        days.setdefault(date, []).append(text)
        self._write_text(path, self._dump_log(days))

    def load_journal(self, pid: str) -> list[JournalEntry]:
        path = self._log_path(pid)
        if not self.fs.exists(path):
            return []
        days = self._parse_log(self._read_text(path))
        return [
            JournalEntry(date=d, body="\n".join(f"- {item}" for item in days[d]))
