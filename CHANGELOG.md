...
- CLI consistency pass:
  - `tool add` uses `--type` (was `--kind`) to match `link add` / `project create`.
  - `init` accepts an optional `--description` (defaults to the name), like `workspace create`.
  - `doc edit` can update the description on its own; the redundant `doc set-description` was removed.
  - `list export` drops the `-o` short flag; use `--output`.
- `search` now scans list names, descriptions, and entries, and honours the global `-p/--project`
  (its own `--project` option was removed to avoid shadowing the global one).
...
- Pluggable storage "sink" selected by a root URL: `file://` (default), `s3://`, and `gcs://` via
  `fsspec`, with `s3` and `gcs` install extras. The same bundle round-trips across all three.
...
- Workspaces and projects, with per-item types: docs, notes, links, lists, assets, tools, tasks,
  and an automatically written day-by-day journal.
...
[Unreleased]: https://github.com/OWNER/totebag/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/OWNER/totebag/releases/tag/v0.1.0
