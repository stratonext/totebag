...
def test_tasks_tools_journal(tmp_path: Path) -> None:
    root = f"file://{tmp_path}/totebag"
    s = Store(root)
    s.init_workspace()
    ...
    s.add_journal(pid, "scaffolded the webhook handler")
    ...
    # completing a task journals it and drops it from the list
    s2.complete_task(pid, task.id, "webhooks handled with idempotency keys")
    assert s2.list_tasks(pid) == []
    journal_text = " ".join(e.body for e in s2.list_journal(pid))
    assert "scaffolded the webhook handler" in journal_text
    assert "webhooks handled" in journal_text

    blob = build_context(s2, s2.get_project(pid))
    for expected in ["stripe-cli", "restore:", "Journal", "webhooks handled"]:
        assert expected in blob
